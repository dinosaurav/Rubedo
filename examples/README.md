...
| [`ray_executor`](ray_executor/) | local Ray cluster (`ray` in the dev group) | expand → burn → polish → verify → reduce | heavy 3-hop Ray CPU workload (SHA stretch + mixer); tune with `RUBEDO_RAY_*`; second run fully reuses |
