...
| [`ray_executor`](ray_executor/) | local Ray cluster (`ray` in the dev group) | expand → map → reduce | same external-executor shape as Dask, with a thin `ObjectRef.future()` pool adapter |
