...
| [`paper_scout`](paper_scout/) | OpenAlex (keyless) | discover → sampled rate-limited fetch → aggregate | `RunScope.sample_n`, `targets=`, `home.runs`, a cautious `12/min` API budget, and sample → inspect → full rollout reuse |
