...
def show_mat_cmd(args):
    with get_session() as session:
        mat = (
            ...
            .filter_by(output_address=args.address)
            .first()
        )
        if not mat:
        ...
        parents = (
            session.query(Materialization)
            .join(
                MaterializationEdge, Materialization.id == MaterializationEdge.parent_id
            )
            .filter(MaterializationEdge.child_id == mat.id)
            .all()
        )
        children = (
            session.query(Materialization)
            .join(
                MaterializationEdge, Materialization.id == MaterializationEdge.child_id
            )
            .filter(MaterializationEdge.parent_id == mat.id)
            .all()
        )
    ...
    with get_session() as session:
        ...
        if args.verbose:
            ...
            if coords:
                ...
                for c in coords:
                    print(
                        f"  {c.coordinate:<30} {c.step_name:<15} {c.status:<10} {c.output_address or '-'}"
                    )
...
def explain_cmd(args):
    ...
    for step in topo_steps:
        ...
        output_address = compute_output_address(
            step.name, step.version, input_hash, config_hash
        )
        ...
        with get_session() as session:
            ...
            if mat:
                parent_mats[step.name] = FakeMat(
                    mat.output_content_hash, mat.output_address
                )
            else:
...
def main():
    ...
    proc_parser = subparsers.add_parser(
        "processors", help="Manage pipelines/processors"
    )
    ...
    run_parser.add_argument(
        "--force", help="Force recomputation (ignore cache)", action="store_true"
    )
    run_parser.add_argument("--folder", help="Override the source folder", default=None)
    run_parser.add_argument(
        "--workers", help="Override the number of workers", type=int, default=None
    )
    run_parser.set_defaults(func=run_cmd)

    explain_parser = subparsers.add_parser(
        "explain", help="Explain addressing formula for a file"
    )
    explain_parser.add_argument("processor_id", help="The ID of the pipeline")
    explain_parser.add_argument(
        "coordinate", help="The file coordinate relative to the folder"
    )
    explain_parser.add_argument("--step", help="Filter by step name", default=None)
    explain_parser.set_defaults(func=explain_cmd)

    show_mat_parser = subparsers.add_parser(
        "show-materialization", help="Show materialization by output address"
    )
    ...
    show_run_parser = subparsers.add_parser(
        "show-run", help="Show details of a specific run"
    )
    show_run_parser.add_argument("run_id", help="The ID of the run")
    show_run_parser.add_argument(
        "--verbose", action="store_true", help="Show coordinate details"
    )
    show_run_parser.add_argument(
        "--status", help="Filter coordinates by status", default=None
    )
    ...
    show_events_parser = subparsers.add_parser(
        "show-events", help="Show event log of a specific run"
    )
    show_events_parser.add_argument("run_id", help="The ID of the run")
    show_events_parser.set_defaults(func=show_events_cmd)

    show_dag_parser = subparsers.add_parser(
        "show-dag", help="Show the DAG steps and edges of a pipeline"
    )
