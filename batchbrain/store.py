...
def _get_object_path(output_address: str) -> str:
    return os.path.join(
        OBJECTS_DIR, output_address[:2], output_address[2:4], output_address
    )
...
def stage_and_commit(
    ...
    coordinate: str,
    output_address: str,
    result: Union[str, bytes, ProcessResult, Any],
) -> Tuple[str, str]:
    init_store()
    ...
    if isinstance(value, bytes):
        ...
        raw_data = json.dumps(value, sort_keys=True, separators=(",", ":")).encode(
            "utf-8"
        )
