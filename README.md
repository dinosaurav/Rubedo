...
- **`reduce`** (N:1) — fan in over all a parent's surviving lanes: `@step(shape="reduce")` receives `{lane: value}` and returns one output. Add `group_key="field"` to fan in *per group* instead — one output per value of an indexed field, keyed by that value. By default, it drops failed parent lanes and proceeds with what passed (`on_failed="use_passed"`).
...
- **`join`** — an N-way equijoin across multiple sources, matched on an indexed field. By default, it proceeds despite failed parents (`on_failed="use_passed"`):
