...
from batchbrain.registry import step, pipeline, clear_registry, get_processor
from batchbrain.runner import topological_sort
from batchbrain.cli import run_cmd
from argparse import Namespace
from batchbrain.models import RunCoordinateStatus, Materialization, MaterializationEdge, Run
from batchbrain.hashing import hash_json
import uuid
...
def setup_teardown():
    ...
    batchbrain.db.SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=batchbrain.db.engine)
...
def test_linear_dag():
    ...
    p = pipeline(id="p1", name="p1", folder=TEST_FOLDER, steps=[read, upper])
    
    create_file("f1.txt", "hello")
    create_file("f2.txt", "world")
    
    run_cmd(Namespace(processor_id="p1", inputs=None, force=False, folder=None, workers=1))
    ...
    with get_session() as session:
        ...
        # Check outputs
        rc_read = session.query(RunCoordinateStatus).filter_by(coordinate="f1.txt", step_name="read").first()
        read_f1 = session.get(Materialization, rc_read.materialization_id)
        assert read_f1 is not None
        
        rc_upper = session.query(RunCoordinateStatus).filter_by(coordinate="f1.txt", step_name="upper").first()
        upper_f1 = session.get(Materialization, rc_upper.materialization_id)
        assert upper_f1 is not None
        
        # Check edges
        edge = session.query(MaterializationEdge).filter_by(parent_id=read_f1.id, child_id=upper_f1.id).first()
...
def test_cache_hit():
    ...
    p = pipeline(id="p2", name="p2", folder=TEST_FOLDER, steps=[read])
    
    create_file("f1.txt", "hello")
    run_cmd(Namespace(processor_id="p2", inputs=None, force=False, folder=None, workers=1))
    ...
    with get_session() as session:
        ...
        statuses = session.query(RunCoordinateStatus).order_by(RunCoordinateStatus.id.desc()).limit(1).all()
...
def test_dag_blocked_on_failure():
    ...
    p = pipeline(id="p3", name="p3", folder=TEST_FOLDER, steps=[read, upper])
    
    create_file("f1.txt", "hello")
    
    run_cmd(Namespace(processor_id="p3", inputs=None, force=False, folder=None, workers=1))
    
    with get_session() as session:
        rc_read = session.query(RunCoordinateStatus).filter_by(step_name="read").order_by(RunCoordinateStatus.id.desc()).first()
        assert rc_read.status == "failed"
        
        rc_upper = session.query(RunCoordinateStatus).filter_by(step_name="upper").order_by(RunCoordinateStatus.id.desc()).first()
