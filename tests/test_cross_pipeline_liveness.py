...
def make_pipe(name, folder, retention=None):
    return pipeline(name=name, steps=[make_scan(folder), extract], retention=retention, home=TEST_HOME)
...
def test_identical_payloads_mint_identical_lane_keys_across_pipelines():
    ...
    scan_a = [r for r in TEST_HOME.lanes.all_filled_rows()
              if r["pipeline_id"] == "pipe-a" and r["step_name"] == "scan"
              and r["lane_key"] != "@root"]
    scan_b = [r for r in TEST_HOME.lanes.all_filled_rows()
    ...
    extract_a = [r for r in TEST_HOME.lanes.all_filled_rows()
                 if r["pipeline_id"] == "pipe-a" and r["step_name"] == "extract"]
    extract_b = [r for r in TEST_HOME.lanes.all_filled_rows()
...
def test_invalidating_pipeline_a_leaves_pipeline_b_reuse_intact():
    ...
    res = invalidate(Selection(pipeline_id="pipe-a"), reason="redo pipe-a", home=TEST_HOME)
    # scan's child lane + scan's cache anchor + extract, pipe-a only.
    assert res["invalidated_count"] == 3

    with TEST_HOME.session() as s:
        b_addrs = {
            r["address"] for r in TEST_HOME.lanes.all_filled_rows()
...
def test_retention_pruning_pipeline_a_does_not_recompute_pipeline_b():
    ...
    with TEST_HOME.session() as s:
        ...
        a_rows = [r for r in TEST_HOME.lanes.all_filled_rows() if r["pipeline_id"] == "pipe-a"]
        ...
        b_rows = [r for r in TEST_HOME.lanes.all_filled_rows() if r["pipeline_id"] == "pipe-b"]
