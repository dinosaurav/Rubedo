...
def test_append_filled_and_find():
    append_filled(PIPE, STEP, "@root#0", "addr_1", "ih_a", "ch_0", "json", RUN1)
...
def test_find_latest_filled_missing_lane():
    append_filled(PIPE, STEP, "@root#0", "addr_2", "ih_a", "ch_0", "json", RUN1)
...
def test_find_latest_filled_with_input_hash_filter():
    ...
    append_filled(PIPE, STEP, "@root#0", "addr_3", "ih_v1", "ch_v1", "json", RUN1)
    append_filled(PIPE, STEP, "@root#0", "addr_4", "ih_v2", "ch_v2", "json", RUN1,
...
def test_latest_by_ts_wins():
    ...
    append_filled(PIPE, STEP, "@root#0", "addr_5", "ih_a", "ch_old", "json", RUN1,
                  ts=old_ts)
    append_filled(PIPE, STEP, "@root#0", "addr_6", "ih_a", "ch_new", "json", RUN2,
                  ts=new_ts)
    row = find_latest_filled(PIPE, STEP, "@root#0")
...
def test_find_latest_absent_lane():
    ...
    append_filled(PIPE, STEP, "@root#0", "addr_8", "ih_a", "ch_0", "json", RUN1)
...
def test_find_latest_returns_most_recent():
    ...
    append_filled(PIPE, STEP, "@root#0", "addr_old", "ih_a", "ch_old", "json",
                  RUN1, ts=_ts(minutes_ago=10))
    append_filled(PIPE, STEP, "@root#0", "addr_new", "ih_a", "ch_new", "json",
...
def test_get_all_lane_keys():
    append_filled(PIPE, STEP, "@root#0", "addr_11", "ih_a", "ch_0", "json", RUN1)
    append_filled(PIPE, STEP, "@root#1", "addr_12", "ih_b", "ch_1", "json", RUN1)
...
def test_get_filled_rows():
    append_filled(PIPE, STEP, "@root#0", "addr_13", "ih_a", "ch_0", "json", RUN1,
                  ts=_ts(minutes_ago=5))
    append_filled(PIPE, STEP, "@root#0", "addr_14", "ih_a", "ch_1", "json", RUN2,
                  ts=_ts(minutes_ago=0))  # newer generation
    append_filled(PIPE, STEP, "@root#1", "addr_15", "ih_b", "ch_2", "json", RUN1)
...
def test_flush_persists_to_disk():
    append_filled(PIPE, STEP, "@root#0", "addr_16", "ih_a", "ch_0", "json", RUN1)
...
def test_flush_all_writes_every_step():
    append_filled(PIPE, STEP, "@root#0", "addr_17", "ih_a", "ch_0", "json", RUN1)
    append_filled(PIPE, "other_step", "@root#1", "addr_other", "ih_b", "ch_1", "json",
                  RUN1)
    flush_all()
    clear_run_buffers()
...
def test_flush_accumulates_across_runs():
    ...
    append_filled(PIPE, STEP, "@root#0", "addr_18", "ih_a", "ch_v1", "json", RUN1,
                  ts=_ts(minutes_ago=10))
    flush_step(PIPE, STEP)
    clear_run_buffers()

    append_filled(PIPE, STEP, "@root#0", "addr_19", "ih_a", "ch_v2", "json", RUN2,
...
def test_in_memory_buffer_visible_before_flush():
    ...
    append_filled(PIPE, STEP, "@root#0", "addr_20", "ih_a", "ch_0", "json", RUN1)
...
def test_compact_keeps_latest_per_lane():
    append_filled(PIPE, STEP, "@root#0", "addr_21", "ih_a", "ch_v1", "json", RUN1,
                  ts=_ts(minutes_ago=10))
    append_filled(PIPE, STEP, "@root#0", "addr_22", "ih_a", "ch_v2", "json", RUN2,
                  ts=_ts(minutes_ago=0))
    append_filled(PIPE, STEP, "@root#1", "addr_23", "ih_b", "ch_3", "json", RUN1)
    flush_step(PIPE, STEP)
    clear_run_buffers()
...
def test_compact_drops_unkept_lanes():
    append_filled(PIPE, STEP, "@root#0", "addr_24", "ih_a", "ch_0", "json", RUN1)
    append_filled(PIPE, STEP, "@root#1", "addr_25", "ih_b", "ch_1", "json", RUN1)
    flush_step(PIPE, STEP)
    clear_run_buffers()
...
def test_compact_removes_file_when_nothing_kept():
    append_filled(PIPE, STEP, "@root#0", "addr_26", "ih_a", "ch_0", "json", RUN1)
    flush_step(PIPE, STEP)
    clear_run_buffers()
...
def test_different_pipelines_isolated():
    append_filled("pipe_a", STEP, "@root#0", "addr_a", "ih_a", "ch_a", "json", RUN1)
    append_filled("pipe_b", STEP, "@root#0", "addr_b", "ih_b", "ch_b", "json", RUN1)
    assert find_latest_filled("pipe_a", STEP, "@root#0")["output"] == "ch_a"
    assert find_latest_filled("pipe_b", STEP, "@root#0")["output"] == "ch_b"
