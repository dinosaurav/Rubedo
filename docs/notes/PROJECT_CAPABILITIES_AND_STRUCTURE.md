...
### Key Capabilities:
1. **DAG Pipeline Framework:** Developers define multi-step DAG pipelines with `@step` and `pipeline()` (or fluently via `PipelineBuilder`). Four shapes: `map` (1:1 per lane), `reduce` (N:1 fan-in over a parent's surviving lanes; `group_key="field"` fans in per indexed-field value), `expand` (1:N — a step `yield`s a payload per item, minting content-addressed lanes, cached against its parent so a scrape runs once), and `join` (N-way equijoin across sources matched on indexed fields, minting `a|b` pair lanes). A pipeline can span multiple sources (`sources={name: Source}`, root steps pick one via `@step(source=...)`).
2. **Pluggable Coordinate Sources:** A `Source` abstraction enumerates coordinates ("lanes") and loads their payloads. `FolderSource` (files, coordinate = relative path) and `CsvSource`/`TableSource` (rows) ship built in; every row is a content-addressed lane (`row-<hash>`; identical rows collapse) — no lane key, `@step(index=[...])` a field to search by it. `TableSource` adds an optional `batch_size` streaming mode (whose `key=` is only the row re-fetch handle). `folder=`/`source=` are single-source sugar; `sources={name: Source}` declares several; or use no source and a root `@step(shape="expand")`.
3. **Durable Materialization & Invalidation:** The engine treats processing as a durable, content-addressed materialization pipeline. Re-runs purely scan and process current active coordinates, skipping recomputation if the input hash, `params`, and code identity (`version`, and source hash when `code="auto"`) haven't changed.
4. **Durable Run Ledger & Lineage:** The system records exhaustive telemetry for each run. It creates a robust event ledger (`RunEvent`) and computes per-coordinate status summaries (`RunCoordinateStatus`) categorizing work as `created`, `reused`, `failed`, `blocked`, or `filtered` (a step declined the lane via `Filtered(...)` — a cached, first-class verdict). `MaterializationEdge` models persist parent-child lineage.
5. **Step Policies for Flaky/Non-Idempotent Work:** `retries`/`retry_on`/`retry_delay`/`retry_backoff`, `rate_limit` (paced across all workers), `stale_after` (TTL-based recompute), `assertions` (callables to validate outputs), and `skip_cache` (an in-memory, never-materialized util whose identity fuses into its consumers' cache keys) — built for LLM calls and scraping, not just deterministic transforms.
...
9. **API Server:** A read-only FastAPI server exposing pipelines, runs, materializations, lineage, and current outputs to the frontend. It never imports user pipeline code.
10. **Web UI:** A React + Vite dashboard for browsing runs, outputs, and lineage, including deep search of specific values across an entire run. It is purely read-only. Runs and invalidations are triggered from library code (`rubedo.run` and `rubedo.invalidate`), not the UI.
...
- **Run Phases (`planning.py`, `execution.py`, `ledger.py`, `runner.py`)**: read-only planning (decisions, addresses, staleness/drift), DB-free execution (thread/process pool, retries, rate limits, ephemeral utils), all ledger writes (statuses, events, generations), and the `run()`/`plan()` orchestrators. Each run snapshots its pipeline definition into the ledger.
...
- The web app talks to the `rubedo` FastAPI backend (`src/api.ts`) to provide a purely read-only visual dashboard of runs, materializations, lineage, and current outputs.
