...
### `/rubedo/`
This is the core Python backend package containing the execution engine, database logic, and API. There is no CLI (see `docs/TODO.md` item 2 for that as a future direction) — pipelines run from library code (`rubedo.run`/`rubedo.plan`).
